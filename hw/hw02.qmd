...
In this problem, we will examine the posterior distribution of $\theta$ given these data, under a prior distribution for $\theta$ having density of the form $p(\theta) = c \theta^{a-1} e^{-b\theta}$, where $c$ is a constant that depends on $a$ and $b$ but not $\theta$. For this problem, we will set $a = 2$ and $b = 1/5$. Assume the same sampling model as in exercise 3.
